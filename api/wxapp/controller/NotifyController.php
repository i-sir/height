...
class NotifyController extends AuthController
{
     ...
     * 微信支付回调 测试
     *
     *   test_environment: http://height.ikun:9090/api/wxapp/notify/wx_pay_notify_test?pay_num=1000
     *   official_environment: https://xcxkf173.aubye.com/api/wxapp/notify/wx_pay_notify_test?pay_num=1000
    ...
    public function processOrder($pay_num)
    {
        ...
        if ($pay_info['order_type'] == 40) {
            ...
            $order_info = $CourseOrderModel->where($map100)->find();//查询订单信息

            //发放优惠券,发放佣金
            $InitController->payCourseOrderAccomplish($this->getParams($order_num), $pay_info['user_id'], $pay_num);
        }
