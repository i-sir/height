 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\CourseSetController();
 * )
 */
...
class CourseSetController extends AdminBaseController
{
    ...
    protected function base_edit()
    {
        ...
        $map[] = ['is_show', '=', 1];
        $this->assign("course_list", $CourseInit->get_list($map));
    }
    ...
    public function index()
    {
        ...
        /** 导出数据 **/
        if ($params["is_export"]) $CourseSetInit->export_excel($where, $params);


        /** 查询数据 **/
        $result = $CourseSetInit->get_list_paginate($where, $params);
    ...
    public function add_post()
    {
        ...
        /** 检测参数信息 **/
        $validateResult = $this->validate($params, 'CourseSet');
        if ($validateResult !== true) $this->error($validateResult);


        /** 插入数据 **/
        $result = $CourseSetInit->admin_edit_post($params);
        if (empty($result)) $this->error("失败请重试");

        $this->success("保存成功", "index{$params['class_id']}{$this->params_url}");
    }
    ...
    public function find()
    {
        ...
        $result                  = $CourseSetInit->get_find($where, $params);
    ...
    public function edit_post()
    {
        ...
        /** 提交数据 **/
        $result = $CourseSetInit->admin_edit_post($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function edit_post_two()
    {
        ...
        /** 提交数据 **/
        $result = $CourseSetInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function audit_post()
    {
        ...
        $params["InterfaceType"] = "admin";//接口类型
        $params["DataFormat"]    = "find";//数据格式,find详情,list列表
        $item                    = $CourseSetInit->get_find($where);
    ...
    public function delete()
    {
        ...
        /** 删除数据 **/
        $result = $CourseSetInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $CourseSetInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $CourseSetInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
