...
class ShopInit extends Base
{
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        if ($this->InterfaceType == 'api') {
            ...
            /** 处理富文本 **/
            if ($item['content']) $item['content'] = htmlspecialchars_decode(cmf_replace_content_file_url($item['content']));//介绍
            ...
            /** admin处理文件 **/
            if ($item['images']) $item['images'] = $this->getParams($item['images']);//轮播图
    ...
    public function api_edit_post($params = [], $where = [])
    {
        ...
        /** 接口提交,处理数据 **/
        if ($params['images']) $params['images'] = $this->setParams($params['images']);//轮播图
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            //如传入id,根据id编辑数据
            $params["update_time"] = time();
            $result                = $ShopModel->where("id", "=", $params["id"])->strict(false)->update($params);
            //if($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
    ...
    public function edit_post_two($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $ShopModel->strict(false)->insert($params);
        }
    ...
    public function export_excel($where = [], $params = [])
    {
        ...
        $result = $ShopInit->get_list($where, $params);
        ...
        $Excel->excelExports($result, $headArrValue, ["fileName" => "店铺管理"]);
    }
