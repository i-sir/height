...
class AvatarInit extends Base
{
    ...
    protected $DataFormat    = "find";//数据格式,find详情,list列表
        ...
        {
            $AvatarInit  = new \init\AvatarInit();//头像库   (ps:InitController)
    ...
    */
    public function get_join_list($where=[], $params = [])
    {
        ...
        */
        public function edit_post($params,$where=[])
        {
          ...
          if(!empty($where)){
                ...
                $result                = $AvatarModel->where($where)->strict(false)->update($params);
                //if ($result) $result = $item["id"];
            } elseif (!empty($params["id"])) {
                //如传入id,根据id编辑数据
                $params["update_time"] = time();
                $result                = $AvatarModel->where("id","=",$params["id"])->strict(false)->update($params);
                //if($result) $result = $item["id"];
           } else {
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $AvatarModel->strict(false)->insert($params,true);
            }
        ...
        */
        public function edit_post_two($params,$where=[])
        {
            ...
            if(!empty($where)){
                ...
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $AvatarModel->strict(false)->insert($params);
            }
        ...
        */
        ...
        if ($type == 1) $result = $AvatarModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $AvatarModel->destroy($id,true);//真实删除
        ...
        */
        public function batch_post($id, $params=[])
        {
            ...
            $params["update_time"] = time();
            $result = $AvatarModel->where($where)->strict(false)->update($params);//修改状态
        ...
        */
        public function list_order_post($list_order,$params=[])
        {
            ...
            foreach ($list_order as $k => $v) {
                $where   = [];
                $where[] = ["id", "=", $k];
                $result  = $AvatarModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
            }
        ...
        */
        public function export_excel($where = [], $params = [])
        {
            ...
            $result       = $AvatarInit->get_list($where, $params);
            ...
            $Excel->excelExports($result, $headArrValue, ["fileName" => "头像库"]);
        }
